...
public class BlockL extends Block {
    ...
    public BlockL(Field field){
        ...
        switch (currentState){
            case 0:
                ...
                locations.add(3, new Location(tempx + 1, 2, color));
                break;
            case 1:
                locations.add(0, new Location(tempx + 2, 0, color));
                locations.add(1, new Location(tempx + 1, 0, color));
                locations.add(2, new Location(tempx, 0, color));
                locations.add(3, new Location(tempx, 1, color));
                break;
            case 2:
                locations.add(0, new Location(tempx + 2, 2, color));
                locations.add(1, new Location(tempx + 2, 1, color));
                locations.add(2, new Location(tempx + 2, 0, color));
                locations.add(3, new Location(tempx + 1, 0, color));
                break;
            case 3:
                locations.add(0, new Location(tempx, 2, color));
                locations.add(1, new Location(tempx + 1, 2, color));
                locations.add(2, new Location(tempx + 2, 2, color));
    ...
    public boolean rotate(MyCube[][] allCubes) {
        switch (currentState){
            case 3:
                if(allCubes[locations.get(0).getY() - 2][locations.get(0).getX()] == null &&
                        allCubes[locations.get(1).getY() - 1][locations.get(1).getX() - 1] == null &&
                        allCubes[locations.get(2).getY()][locations.get(2).getX() - 2] == null &&
                        allCubes[locations.get(3).getY() + 1][locations.get(3).getX() - 1] == null) {
                    locations.get(0).setLocation(0, -2);
                    locations.get(1).setLocation(-1, -1);
                    locations.get(2).setLocation(-2, 0);
                    locations.get(3).setLocation(-1, 1);
                    currentState = (currentState + 1) % maxState;
                    return true;
                }
                return false;
            case 0:
                if(allCubes[locations.get(0).getY()][locations.get(0).getX() + 2] == null &&
                        allCubes[locations.get(1).getY() - 1][locations.get(1).getX() + 1] == null &&
                        allCubes[locations.get(2).getY() - 2][locations.get(2).getX()] == null &&
                        allCubes[locations.get(3).getY() - 1][locations.get(3).getX() - 1] == null) {
                    locations.get(0).setLocation(2, 0);
                    locations.get(1).setLocation(1, -1);
                    locations.get(2).setLocation(0, -2);
                    locations.get(3).setLocation(-1, -1);
                    currentState = (currentState + 1) % maxState;
                    return true;
                }
